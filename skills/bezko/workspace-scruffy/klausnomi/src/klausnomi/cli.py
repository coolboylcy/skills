...
def format_nomi(nomi: Nomi) -> str:
    ...
    Args:
        nomi: The Nomi object to format.
        
    Returns:
        Formatted string with name, UUID, and gender.
    """
...
async def handle_nomi_show(client: NomiClient, args: argparse.Namespace) -> int:
    ...
    Args:
        client: The NomiClient instance.
        args: Parsed command arguments.
        
    Returns:
        Exit code (0 for success, 1 if Nomi not found).
    """
    try:
        nomi = await client.get_nomi(args.uuid)
        if args.json:
